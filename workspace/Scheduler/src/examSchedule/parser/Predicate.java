 ...
 */
public enum ParamType { STRING, SET, VECTOR, LONG, UNDEFINED }
 ...
 * (type x value).  
 * The recognized types are recognized by their form:
 * <UL>
 * <LI>SET: "{" comma-separated-list "}"
 * <LI>VECTOR: "[" comma-sparated-list "]"
 * <LI>STRING: """ characters """
 * <LI>STRING: characters-without-whitespace
 * <LI>LONG: [-]{0-9}*
 * </UL>
	...
	while ((paramEnd = scanFor(list,++cur,delim))!=-1) {
		...
		switch (list.charAt(cur)) {
			case '{':
				...
				break;
			case '[':
				obj = scanList(list,curInt.setValue(cur),']',',');
				cur = curInt.getValue();
				t = ParamType.VECTOR;
				// paramEnd might have been wrong if there was more than one element in the set
				paramEnd = scanFor(list,cur,delim);  
...
/**
 * Returns the value of a vector parameter at index <code>index</code>
 * as a <code>Vector&lt;Pair&lt;ParamType,Object&lt;&lt;</code>.  
 * Thus, nested sets/vectors are allowed.
 * If the index is out of range or the parameter is not of a vector
 * type, returns <code>null</code>. 
 * @param index the index of the parameter to evaluate
 * @return a <code>Vector</code> representing the value of parameter at the index, or null.
 */
@SuppressWarnings("unchecked")
public Vector<Pair<ParamType,Object>> getVectorParam(int index) {
	if (index < params.size()) {
		Pair<ParamType,Object> p =  params.get(index);
		if (p.getKey() == ParamType.VECTOR) {
			return (Vector<Pair<ParamType,Object>>)p.getValue();
		} else return null;
	} else return null;
}
